...
class OperatorRepository : public BaseRepository<OperatorDAO> {
public:
    explicit OperatorRepository(std::shared_ptr<Database> db)
        : BaseRepository(db) {
        ensure_table();
    }
    ...
    std::optional<OperatorDAO> get_by_uid(const UUID& uid) override {
        std::optional<OperatorDAO> opt;
        sqlite3* h = db_->handle();
        sqlite3_stmt* stmt = nullptr;
        const char* sql = "SELECT operator_id, operator_uid, username, password, clearance "
                         "FROM operators WHERE operator_uid = ? LIMIT 1;";
    ...
    std::optional<OperatorDAO> get_by_username(const std::string& username) {
        std::optional<OperatorDAO> opt;
        sqlite3* h = db_->handle();
        sqlite3_stmt* stmt = nullptr;
        const char* sql = "SELECT operator_id, operator_uid, username, password, clearance "
                         "FROM operators WHERE username = ? LIMIT 1;";
